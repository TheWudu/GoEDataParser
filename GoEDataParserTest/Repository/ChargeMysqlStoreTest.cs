...
public class ChargeMysqlStoreTest
{
    ...
    private async Task<Charge> CreateCharge(int day, string? id = null)
    {
        Charge charge = new()
        {
    ...
    {
        Charge charge = await CreateCharge(10);

        Assert.Equal(charge, await _store.FindBy(e => e.SessionId == "sessionId_1234_10"));
    }

    [Fact]
    public async Task FindByNotFound()
    {
        Assert.Null(await _store.FindBy(e => e.SessionId == "not-existing-id"));
    }

    [Fact]
    public async Task FindByStartDate()
    {
        _ = await CreateCharge(10);
        Charge charge = await CreateCharge(22);
        _ = await CreateCharge(23);
    ...
    public async Task ReadAll(int amount)
    {
        ...
        List<Charge> charges = await _store.ReadAll();
    ...
    public async Task UpdateExisting()
    {
        ...
        var updatedCharge = await _store.Find(id);
