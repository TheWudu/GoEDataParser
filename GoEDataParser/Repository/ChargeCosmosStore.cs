...
{
    public class ChargeCosmosStore(string dbHost, string dbName)
        : GenericCosmosStore<Charge>(dbHost, dbName, "charges"),
            IChargeStore
    {
        ...
        {
            var list = Dataset.Where(e => true).OrderBy(e => e.StartTime).ToListAsync().Result;
